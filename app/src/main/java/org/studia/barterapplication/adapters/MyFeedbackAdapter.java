...
public class MyFeedbackAdapter extends RecyclerView.Adapter<MyFeedbackAdapter.FeedbackViewHolder> {
     ...
     * @param context
     */
    public MyFeedbackAdapter(ArrayList<String> names, ArrayList<String> feedbacks, ArrayList<String> userIDs, Context context) {
        this.names = names;
    ...
    public void onBindViewHolder(@NonNull FeedbackViewHolder holder, int position) {
        ...
        holder.feedbackUsernameTextView.setText("Od: " + names.get(position));
        // Set the feedback for the feedback text view
        holder.feedbackTextView.setText("Wiadomość: " + feedbacks.get(position));
        ...
        holder.feedbackVisitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                db.collection("usersObj").whereEqualTo("documentId", userIDs.get(position)).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        for (DocumentSnapshot documentSnapshot : task.getResult() ) {

                            currentSeller = documentSnapshot.toObject(User.class);
                            Toast.makeText(context, "Otwarto profil", Toast.LENGTH_LONG).show();

                            // Set the currentSeller for the Barter class so that we can pull this object in the OtherUsersProfile page
                            // from the barter class
